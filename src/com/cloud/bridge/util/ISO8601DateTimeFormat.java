...
/**
 * @author John Zucker
 * Format and parse an ISO 8601 DateTimeFormat for use in XML documents.
 ...
 * Must implement two behaviors inherited from DateFormat: parse and format.
...
public class ISO8601DateTimeFormat extends DateFormat {
	...
	private static final long serialVersionUID = 7388260211953189670L;
	   ...
	   * or throws a IllegalArgumentException if not formatted as ISO8601
	   */
	  @Override
	  public Date parse(String dateString, ParsePosition pos) {
	    ...
	    try {
	      ...
	      if (dateString.charAt(p) != '-') {
	        throw new IllegalArgumentException("Not a ISO8601 format date after YYYY " + dateString);
	      }
	      p++;
	    // Assign value of MM
	      ...
	      p += 2;
	      if (dateString.charAt(p) != '-') {
		        throw new IllegalArgumentException("Not a ISO8601 format date after MM " + dateString);
	      }
	      p++;
	    // Asign value of dd
	      ...
	      if (dateString.charAt(p) != 'T') {
		        throw new IllegalArgumentException("Not a ISO8601 format date after DD " + dateString);
	      }
	      p++;
	    // Assign value of hh
	      ...
	      p += 2;
	      if (dateString.charAt(p) != ':') {
		        throw new IllegalArgumentException("Not a ISO8601 format date after hh " + dateString);
	      }
	      p++;
       // Assign value of mm
