...
public class BareMetalResourceBase implements ServerResource {
	...
	public boolean configure(String name, Map<String, Object> params) throws ConfigurationException {
		...
		try {
			_memCapacity = Long.parseLong((String)params.get("memCapacity")) * 1024L * 1024L;
			_cpuCapacity = Long.parseLong((String)params.get("cpuCapacity"));
			_cpuNum = Long.parseLong((String)params.get("cpuNum"));
		} catch (NumberFormatException e) {
			throw new ConfigurationException(String.format("Unable to parse number of CPU or memory capacity " +
					"or cpu capacity(cpu number = %1$s memCapacity=%2$s, cpuCapacity=%3$s", (String)params.get("cpuNum"),
					(String)params.get("memCapacity"), (String)params.get("cpuCapacity")));
		}
	...
	//TODO: do we need this in pingcommand?
	protected Map<String, State> fullSync() {
		...
		if (_vmName != null) {
			...
			if (isPowerOn(interpreter.getLines())) {
				changes.put(_vmName, State.Running);
			} else {
				changes.put(_vmName, State.Stopped);
			}
		}
