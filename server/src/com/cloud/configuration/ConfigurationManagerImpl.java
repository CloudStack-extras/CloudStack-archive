...
public class ConfigurationManagerImpl implements ConfigurationManager {
    ...
    private void checkPodAttributes(long podId, String podName, long zoneId, String gateway, String cidr, String startIp, String endIp, boolean checkForDuplicates)
    ...
    public void deletePod(long userId, long podId) throws InvalidParameterValueException, InternalErrorException {
        ...
        saveConfigurationEvent(userId, null, EventTypes.EVENT_POD_DELETE, "Successfully deleted pod with name: " + pod.getName() + " in zone: " + zone.getName() + ".", "podId=" + podId, "dcId="
                + zone.getId());
    }
    ...
    public HostPodVO editPod(long userId, long podId, String newPodName, String gateway, String cidr, String startIp, String endIp) throws InvalidParameterValueException, InternalErrorException {
        ...
        saveConfigurationEvent(userId, null, EventTypes.EVENT_POD_EDIT, "Successfully edited pod. New pod name is: " + newPodName + " and new zone name is: " + zone.getName() + ".",
                "podId=" + pod.getId(), "dcId=" + zone.getId(), "gateway=" + gateway, "cidr=" + cidr, "startIp=" + startIp, "endIp=" + endIp);
    ...
    public HostPodVO createPod(long userId, String podName, long zoneId, String gateway, String cidr, String startIp, String endIp) throws InvalidParameterValueException, InternalErrorException {
        ...
        DataCenterVO zone = _zoneDao.findById(zoneId);
        saveConfigurationEvent(userId, null, EventTypes.EVENT_POD_CREATE, "Successfully created new pod with name: " + podName + " in zone: " + zone.getName() + ".", "podId=" + pod.getId(), "zoneId="
                + zone.getId(), "gateway=" + gateway, "cidr=" + cidr, "startIp=" + startIp, "endIp=" + endIp);
    ...
    @Override
    public DataCenterVO editZone(long userId, long zoneId, String newZoneName, String dns1, String dns2, String internalDns1, String internalDns2, String vnetRange, String guestCidr)
        ...
        if ((dns1 != null && !dns1.equals(zone.getDns1())) || (dns2 != null && !dns2.equals(zone.getDns2())) || (internalDns1 != null && !internalDns1.equals(zone.getInternalDns1()))
                || (internalDns2 != null && !internalDns2.equals(zone.getInternalDns2()))) {
        ...
        saveConfigurationEvent(userId, null, EventTypes.EVENT_ZONE_EDIT, "Successfully edited zone with name: " + zone.getName() + ".", "dcId=" + zone.getId(), "dns1=" + dns1, "dns2=" + dns2,
                "internalDns1=" + internalDns1, "internalDns2=" + internalDns2, "vnetRange=" + vnetRange, "guestCidr=" + guestCidr);
    ...
    @Override
    @DB
    public DataCenterVO createZone(long userId, String zoneName, String dns1, String dns2, String internalDns1, String internalDns2, String vnetRange, String guestCidr)
            throws InvalidParameterValueException, InternalErrorException {
        ...
        saveConfigurationEvent(userId, null, EventTypes.EVENT_ZONE_CREATE, "Successfully created new zone with name: " + zoneName + ".", "dcId=" + zone.getId(), "dns1=" + dns1, "dns2=" + dns2,
                "internalDns1=" + internalDns1, "internalDns2=" + internalDns2, "vnetRange=" + vnetRange, "guestCidr=" + guestCidr);
    ...
    @Override
    public ServiceOfferingVO createServiceOffering(long userId, String name, int cpu, int ramSize, int speed, String displayText, boolean localStorageRequired, boolean offerHA,
            boolean useVirtualNetwork, String tags, String hostTag) {
        ...
        if ((offering = _serviceOfferingDao.persist(offering)) != null) {
            saveConfigurationEvent(userId, null, EventTypes.EVENT_SERVICE_OFFERING_CREATE, "Successfully created new service offering with name: " + name + ".", "soId=" + offering.getId(), "name="
                    + name, "numCPUs=" + cpu, "ram=" + ramSize, "cpuSpeed=" + speed, "displayText=" + displayText, "guestIPType=" + guestIpType, "localStorageRequired=" + localStorageRequired,
                    "offerHA=" + offerHA, "useVirtualNetwork=" + useVirtualNetwork, "tags=" + tags, "host_tag=" + hostTag);
            return offering;
        } else {
            return null;
        }
    ...
    public ServiceOfferingVO updateServiceOffering(long userId, long serviceOfferingId, String name, String displayText, Boolean offerHA, Boolean useVirtualNetwork, String tags) {
        ...
        if (_serviceOfferingDao.update(serviceOfferingId, offering)) {
            ...
            saveConfigurationEvent(userId, null, EventTypes.EVENT_SERVICE_OFFERING_EDIT, "Successfully updated service offering with name: " + offering.getName() + ".", "soId=" + offering.getId(),
                    "name=" + offering.getName(), "displayText=" + offering.getDisplayText(), "offerHA=" + offering.getOfferHA(), "useVirtualNetwork="
                            + (offering.getGuestIpType() == GuestIpType.Virtualized), "tags=" + offering.getTags());
            return offering;
        } else {
            return null;
        }
    ...
    public boolean deleteServiceOffering(long userId, long serviceOfferingId) {
        ...
        if (_serviceOfferingDao.remove(serviceOfferingId)) {
            saveConfigurationEvent(userId, null, EventTypes.EVENT_SERVICE_OFFERING_EDIT, "Successfully deleted service offering with name: " + offering.getName(), "soId=" + serviceOfferingId, "name="
                    + offering.getName(), "displayText=" + offering.getDisplayText(), "offerHA=" + offering.getOfferHA(), "useVirtualNetwork=" + (offering.getGuestIpType() == GuestIpType.Virtualized));
            return true;
        } else {
            return false;
        }
    ...
    @Override
    public VlanVO createVlanAndPublicIpRange(long userId, VlanType vlanType, Long zoneId, Long accountId, Long podId, String vlanId, String vlanGateway, String vlanNetmask, String startIP,
            String endIP, Long domainId) throws InvalidParameterValueException, InternalErrorException {
        ...
        if (newVlanSubnet.equals(guestSubnet)) {
            throw new InvalidParameterValueException("The new VLAN you have specified has the same subnet as the guest network in zone: " + zone.getName()
            ...
            if (vlanId.equals(vlan.getVlanId()) && newVlanSubnet.equals(otherVlanSubnet)) {
                if (!(vlanType.equals(VlanType.DirectAttached) && !vlanId.equals(Vlan.UNTAGGED))) {
                    if (NetUtils.ipRangesOverlap(startIP, endIP, otherVlanStartIP, otherVlanEndIP)) {
                        throw new InvalidParameterValueException("The VLAN with ID " + vlan.getVlanId()
                ...
                if (!vlanGateway.equals(otherVlanGateway)) {
                    throw new InvalidParameterValueException("The VLAN with ID " + vlan.getVlanId() + " has already been added with gateway " + otherVlanGateway
                            + ". Please specify a different VLAN ID.");
                }
            }
        }
        ...
        if (accountId != null && vlanType.equals(VlanType.VirtualNetwork)) {
            if (!savePublicIPRangeForAccount(startIP, endIP, zoneId, vlan.getId(), accountId, _accountDao.findById(accountId).getDomainId())) {
                ...
                throw new InternalErrorException("Failed to save IP range. Please contact Cloud Support."); // It can be Direct
                                                                                                            // IP or Public IP.
            }
        } else if (!savePublicIPRange(startIP, endIP, zoneId, vlan.getId())) {
            deletePublicIPRange(vlan.getId());
            _vlanDao.delete(vlan.getId());
            throw new InternalErrorException("Failed to save IP range. Please contact Cloud Support."); // It can be Direct IP
                                                                                                        // or Public IP.
        }
        ...
        eventMsg += ".";
        saveConfigurationEvent(userId, accountId, EventTypes.EVENT_VLAN_IP_RANGE_CREATE, eventMsg, "vlanType=" + vlanType, "dcId=" + zoneId, "accountId=" + accountId, "podId=" + podId, "vlanId="
                + vlanId, "vlanGateway=" + vlanGateway, "vlanNetmask=" + vlanNetmask, "startIP=" + startIP, "endIP=" + endIP, "domainId=" + domainId);
    ...
    public boolean deleteVlanAndPublicIpRange(long userId, long vlanDbId) throws InvalidParameterValueException {
        ...
        if (success) {
            ...
            saveConfigurationEvent(userId, null, EventTypes.EVENT_VLAN_IP_RANGE_DELETE, eventMsg, "vlanType=" + vlan.getVlanType(), "dcId=" + vlan.getDataCenterId(), "accountId=" + accountId,
                    "podId=" + podId, "vlanId=" + vlan.getVlanId(), "vlanGateway=" + vlan.getVlanGateway(), "vlanNetmask=" + vlan.getVlanNetmask(), "startIP=" + startIP, "endIP=" + endIP);
        }
