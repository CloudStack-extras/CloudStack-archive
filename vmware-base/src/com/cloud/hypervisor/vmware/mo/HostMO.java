...
public class HostMO extends BaseMO implements VmwareHypervisorHost {
	...
	public VirtualMachineMO findVmOnHyperHost(String name) throws Exception {
		...
		if(s_logger.isTraceEnabled())
			s_logger.trace("vCenter API trace - findVmOnHyperHost(). target MOR: " + _mor.get_value() + ", name: " + name);
		
		ServiceUtil serviceUtil = _context.getServiceUtil();
		ManagedObjectReference[] vms = (ManagedObjectReference[])serviceUtil.getDynamicProperty(
			_mor, "vm");
		
		if(vms != null && vms.length > 0) {
			for(ManagedObjectReference morVm : vms) {
				String vmName = (String)serviceUtil.getDynamicProperty(morVm, "name");
				if(vmName.equals(name)) {
					if(s_logger.isTraceEnabled())
						s_logger.trace("vCenter API trace - findVmOnHyperHost() done(successfully).");
					
					return new VirtualMachineMO(_context, morVm);
				}
			}
		}
		
		if(s_logger.isTraceEnabled())
			s_logger.trace("vCenter API trace - findVmOnHyperHost() done(failed).");
		return null;
	}
