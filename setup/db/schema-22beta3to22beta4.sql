...
ALTER TABLE `cloud`.`networks` ADD COLUMN `is_default` int(1) unsigned NOT NULL DEFAULT 0 COMMENT '1 if network is default';
...
ALTER TABLE `cloud`.`service_offering` ADD COLUMN `host_tag` varchar(255) COMMENT 'host tag specified by the service_offering';
...
ALTER TABLE `cloud`.`security_group` DROP FOREIGN KEY `fk_security_group___account_id`;
